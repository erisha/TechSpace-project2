...
router.post('/newProfile', async (req, res) => {
    ...
    // password is hashed, we can create a user
...
//POST to login and create the session
router.post('/login', async (req, res) => {
    ...
    User.findOne({ username })
        ...
        // if they don't we'll redirect to the new profile page
        .catch(error => {
            console.log(error)
            res.json(error)
        })
...
// logout route
// GET that calls destroy on our session
// add an 'are you sure' page if there is time
